...
class Bomb {
  ...
  constructor({ game, ownerId, col, row, power }) {
    this.id = uuidv4();
  ...
  addToBlasted(row, col, direction, destroyed) {
    // Spoils only ever hide inside destructible blocks.
    let spoil = destroyed ? this.craftSpoil(row, col) : null;
  ...
  toJSON() {
    return { id: this.id, ownerId: this.ownerId, col: this.col, row: this.row }
  }
}
